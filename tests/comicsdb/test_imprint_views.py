...
# Imprint History Views
def test_imprint_history_view_requires_login(client, vertigo_imprint):
    """Test that history view requires authentication."""
    resp = client.get(reverse("imprint:history", kwargs={"slug": vertigo_imprint.slug}))
    assert resp.status_code == HTML_REDIRECT_CODE


def test_imprint_history_view_accessible(auto_login_user, vertigo_imprint):
...
def test_imprint_history_uses_correct_template(auto_login_user, vertigo_imprint):
...
def test_imprint_history_shows_creation(auto_login_user, vertigo_imprint):
...
def test_imprint_history_context(auto_login_user, vertigo_imprint):
    """Test that history view context includes object and model name."""
    client, _ = auto_login_user()
    resp = client.get(reverse("imprint:history", kwargs={"slug": vertigo_imprint.slug}))
    assert resp.status_code == HTML_OK_CODE
    assert "object" in resp.context
    assert resp.context["object"] == vertigo_imprint
    assert "model_name" in resp.context
    assert resp.context["model_name"] == "imprint"
