...
def test_authenticated_user_gets_own_pull_list(api_client, pull_list_user, public_pull_list):
    api_client.force_authenticate(user=pull_list_user)
    resp = api_client.get(reverse("api:pull_list-list"))
    assert resp.status_code == status.HTTP_200_OK
    assert resp.data["count"] == 1
    assert resp.data["results"][0]["id"] == public_pull_list.pk


def test_other_user_cannot_see_pull_list(
    api_client, pull_list_user, other_pull_list_user, public_pull_list
):
...
def test_add_series_action_creates_entry(
    api_client, pull_list_user, public_pull_list, pull_list_series
):
    api_client.force_authenticate(user=pull_list_user)
...
def test_add_series_missing_id_returns_400(api_client, pull_list_user, public_pull_list):
    api_client.force_authenticate(user=pull_list_user)
    resp = api_client.post(reverse("api:pull_list-add-series"), data={})
    assert resp.status_code == status.HTTP_400_BAD_REQUEST


def test_add_series_invalid_id_returns_404(api_client, pull_list_user, public_pull_list):
    api_client.force_authenticate(user=pull_list_user)
