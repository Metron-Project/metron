...
def notify(  # noqa: PLR0913
    ...
    url: str | None = None,
    filter_exclude: dict | None = None,
    recipient_users: list | None = None,
    ...
    has a subscription for the key `"comment/response"` and the model instance
    `comment_instance`. The idea would be that the poster of `comment_instance`
    will receive notifications when someone responds to that comment.
    ...
    :param key: A key object which is matched to ``NotificationType.key``.
                Users with a Subscription for that NotificationType will have a Notification
                object created.
    :param url: A URL pointing to your notification.
                If the URL is pointing to your Django project's unique website,
                then add an ``/absolute/url``. However, if the URL should take the user to a
                different website, use a full HTTP scheme, i.e. ``https://example.org/url/``.
    ...
    if app_settings.NYT_ENABLE_CHANNELS:
        from django_nyt import subscribers  # noqa: PLC0415
...
def unsubscribe(
    ...
    """
    Shortcut function to remove all subscriptions related to a notification key and
    either a user or a settings object.

    Unsubscribing does NOT delete old notifications, however the subscription relation
    is nullified. This means that any objects accessed through that relation will become
    inaccessible. This is a particular feature chosen to avoid accidentally allowing
    access to data that may be otherwise have credential-based access.
    ...
    :param object_id: Further narrow down subscriptions to only this object_id
                      (provided a content_type)
    :return: (int, dict) - the return value of Django's queryset.delete() method.
    """

    if user and settings:
        raise ValueError("Cannot apply both User and UserSettings object.")
    if not (user or settings):
        raise ValueError(
